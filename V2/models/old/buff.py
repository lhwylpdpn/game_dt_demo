...
class Buff():
    ...
    def make_invalid(self, hero_or_monster): # 减小buff数值, 使buff失效
        if self.buff_key == "BUFF_ROUND_ACTION": # # 增加移动力{0}格，并持续{0}行动回合
            ...
            hero_or_monster.set_Def(hero_or_monster.Def -  round_up_2_integer(hero_or_monster.DefBase * self.buff_value/100.0))
        elif self.buff_key == "BUFF_ATK": # 增加物理攻击{0}%，并持续{0}行动回合
            hero_or_monster.set_Atk(hero_or_monster.Atk -  round_up_2_integer(hero_or_monster.AtkBase *  self.buff_value/100.0))
        ...
        #     hero_or_monster.set_Hp(hp if hp >= 1 else 1)
            ...
            hero_or_monster.set_MagicalDef(hero_or_monster.MagicalDef - round_up_2_integer(hero_or_monster.MagicalDefBase * self.buff_value/100.0))
    ...
    def make_effective(self, hero_or_monster): # 增加buff数值, 使buff生效
        ...
        if self.buff_key == "BUFF_ROUND_ACTION": # # 增加移动力{0}格，并持续{0}行动回合
            ...
            effect_attr, delta_change = "Def", round_up_2_integer(hero_or_monster.DefBase * self.buff_value/100.0)
            hero_or_monster.set_Def(hero_or_monster.DefBase + delta_change)
        elif self.buff_key == "BUFF_ATK": # 增加物理攻击{0}%，并持续{0}行动回合
            effect_attr, delta_change = "ATK", round_up_2_integer(hero_or_monster.AtkBase * self.buff_value/100.0)
            hero_or_monster.set_Atk(hero_or_monster.AtkBase + delta_change)
        elif self.buff_key == "BUFF_ADD_HP": # 自动回血 {0}机率回血{0}，并持续{0}行动回合
            effect_attr, delta_change = "Hp", round_up_2_integer(hero_or_monster.HpBase * self.buff_value/100.0)
            ...
            effect_attr, delta_change = "MagicalDef", round_up_2_integer(hero_or_monster.MagicalDefBase * self.buff_value/100.0)
