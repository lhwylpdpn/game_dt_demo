...
class Hero():
    ...
    def __init__(self, **kwargs):
        ...
        self.__skid3 =  kwargs.get("skid3", None)          #技能3ID
        self.__skid4 =  kwargs.get("skid4", None)          #技能4ID
    ...
    def dict(self, fields=[]):
        if not fields:
            ...
            fields.extend(["skid0", "skid1", "skid2", "skid3","skid4"])
        data = {}
        if "skills" in fields: fields.remove("skills")
        for each in {"skid0", "skid1", "skid2", "skid3","skid4"}:
            if each in fields:
                fields.remove(each)
                skill_detail = self.__getattribute__(each)
                if skill_detail:
                    data[each] = skill_detail.dict()
